    ...
    dataChannel.onmessage = (event) => {
     ...
     // console.debug(event);
      ...
      if (message && message.event 
        && (message.event == "draw"
          || message.event == "drawRect"
          || message.event == "clear"))
          drawUI.appendPoint(event.data);
      } catch (e) {
         ui.appendMessage(event.data);  
      }
    };
  };
...
export const sendPreOffer = (callType, calleePersonalCode) => {
  ...
  if (
    ...
    callType === constants.callType.WB_PERSONAL_CODE
...
const acceptCallHandler = () => {
  createPeerConnection();
  sendPreOfferAnswer(constants.preOfferAnswer.CALL_ACCEPTED);
  console.log("At accept handler");
  if (store && store.getState() &&
    store.getState().callType == constants.callType.WB_PERSONAL_CODE) {
   
      const wbCanvas = document.getElementById(
      "local_canvas_container"
    );
    wbCanvas.style.pointerEvents = "auto";
      ui.showElementUI(wbCanvas);
    }
  ui.showCallElements(connectedUserDetails.callType);
};
...
export const handlePreOfferAnswer = (data) => {
  console.log("inside handlePreOfferAnswer");
  console.debug(data);
